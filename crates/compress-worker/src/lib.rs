//! The `compress` VGI worker (library).
//!
...
//! the `compress-wasm` crate (the browser build, which serves the same `Worker`
...
/// The catalog name DuckDB sees in `ATTACH 'compress' (TYPE vgi, …)`. Defaults
/// to `compress`, but honors an explicit override so a test harness can rename
