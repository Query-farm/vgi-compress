#!/usr/bin/env bash
# Build the compress VGI worker and run the SQLLogic tests against it using the
...
echo "==> Building compress-worker (release)"
...
WORKER="$REPO_ROOT/target/release/compress-worker"
...
VGI_COMPRESS_WORKER="$WORKER" \
VGI_WORKER_CATALOG_NAME="compress" \
