...
#
# Dispatch the single vgi-compress image into one of its transports:
...
#
# The worker is stateless (pure local-CPU BLOB transform), so there is no /data
...
case "${1:-http}" in
  http)
    ...
    exec compress-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec compress-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec compress-worker "$@"
    ;;
  *)
    exec "$@"
    ;;
