#!/usr/bin/env bash
# Build the browser (`worker:`) build of the compress VGI worker: compress-wasm
# compiled to wasm32-unknown-emscripten, linked by emcc into a MODULARIZE'd Web
# Worker module served over a SharedArrayBuffer channel.
#
...
#   - a nightly toolchain + rust-src: -Z build-std is required because the
...
LIB="target/$TARGET/release/libcompress_wasm.a"
...
#
# MALLOC=mimalloc is thread-caching; emscripten's default dlmalloc serializes
# every allocation on a global lock, which convoys concurrent serve threads.
#
# STACK_SIZE / DEFAULT_PTHREAD_STACK_SIZE: emscripten defaults to a 64 KiB stack,
# which the Arrow encoders + codec paths can overflow (surfaces as a bare "memory
# access out of bounds"). INITIAL_MEMORY must cover main stack + every pthread
# stack up front, or startup stalls growing shared memory while the pool spawns.
echo "==> emcc link"
